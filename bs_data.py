...
def parse_bs_data(bs_data_result, bs_data_logger):
    ...
    for line in lines:
        ...
        if len(station_stats) > 1 and station_stats[0] != "Station Address" and station_stats[0] != "(overall)":
            ...
            try :
                sample["phy_mbps"] = float(station_stats[1])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample["phy_mbps"] = -1

            try :
                sample['data_mbps'] = float(station_stats[2])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['data_mbps'] = -1

            try :
                sample['air_use'] = float(station_stats[3].split("%")[0])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['air_use'] = -1

            try :
                sample['data_use'] = float(station_stats[4].split("%")[0])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['data_use'] = -1

            try :
                sample['retries'] = float(station_stats[5].split("%")[0])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['retries'] = -1

            try :
                sample['bandwidth'] = int(station_stats[6])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['bandwidth'] = -1

            try :
                sample['mcs'] = int(station_stats[7])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['mcs'] = -1

            try :
                sample['nss'] = int(station_stats[8])
            except ValueError:
                bs_data_logger.debug("ValueError : %s", station_stats[0])
                sample['nss'] = -1


            sample['station'] = station_stats[0]
...
def create_bs_data_series(extender, timestamp):
    ...
    for command, interface in bs_data_command:
        ...
        if success is True:
            ...
            bs_data_logger.info("Error on collecting the commande : %s : %s", to_execute, bs_data_result)
